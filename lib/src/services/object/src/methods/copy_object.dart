...
final class CopyObject implements OracleRequestAttributes {
  ...
  final String publicUrlOfCopiedFile,
      jsonData,
      xContentSha256,
      contentLegth,
      contentType;
...
final class CopyObjectDetails implements Details<Map<String, dynamic>> {
  ...
  factory CopyObjectDetails({
    ...
    if (destinationObjectMetadata is Map<String, String> &&
        destinationObjectMetadata.isNotEmpty) {
      source.putIfAbsent(
        'destinationObjectMetadata',
        () => destinationObjectMetadata,
      );
    }
    if (sourceObjectIfMatchETag is String &&
        sourceObjectIfMatchETag.isNotEmpty) {
      source.putIfAbsent(
        'sourceObjectIfMatchETag',
        () => sourceObjectIfMatchETag,
      );
    }
    if (destinationObjectIfMatchETag is String &&
        destinationObjectIfMatchETag.isNotEmpty) {
      source.putIfAbsent(
        'destinationObjectIfMatchETag',
        () => destinationObjectIfMatchETag,
      );
    }
    if (destinationObjectIfNoneMatchETag is String &&
        destinationObjectIfNoneMatchETag.isNotEmpty) {
      source.putIfAbsent(
        'destinationObjectIfNoneMatchETag',
        () => destinationObjectIfNoneMatchETag,
      );
    }
    if (destinationObjectStorageTier is String &&
        destinationObjectStorageTier.isNotEmpty) {
      source.putIfAbsent(
